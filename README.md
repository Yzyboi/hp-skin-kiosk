...
design + initials + accent colour), a Customer Info screen that
...
field - reference ID, store/SKU/design/initials/accent colour, the
...
## Adding real SKUs, designs, and accent colors
...
- Designs are no longer static config either - manage them from the admin
  ...
  is on) picks up the customer's chosen accent.
