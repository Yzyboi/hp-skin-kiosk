...
a binary SVG asset plus hand-tuned placement coordinates, so the admin
dashboard's Designs section manages them one at a time through a
dedicated form instead:

- **Name** - shown to customers on the "Choose Your Design" screen.
- **SVG artwork** - stored and served **exactly as uploaded**. Nothing in
  this pipeline re-encodes, minifies, recolors, or rescales it (beyond
  fitting it to each SKU's aspect ratio at render time, same as any
  `background-size: cover` image) - the artwork a customer sees is
  byte-for-byte what was uploaded, and it stays that way through every
  customization a customer applies on top.
- **Zone** - a percentage rectangle (0-100, relative to the artwork's own
  bounding box) where the customer's initials render: `X%`, `Y%`,
  `Width%`, `Height%`, text `Align` (left/center/right), and either
  "follows the customer's Accent Colour" or a fixed hex colour.
  Percentage-based placement holds across every SKU's aspect ratio.
- **Motif Zone** - a second percentage rectangle where the customer's
  chosen Motif icon renders, tinted with their Accent Colour.

The Design ID is auto-generated by slugifying the Name (same approach as
SKUs). Using "Edit" on an existing design updates its zone/motif-zone
fields in place and optionally replaces the artwork; the artwork can be
left as-is by not choosing a new file. Deleting a design removes both its
JSON record and its SVG file - like Stores/SKUs, deletion is a separate,
explicit "Delete" action and never happens implicitly.

Design artwork lives under `DATA_DIR/design-assets/` (served at
`/design-assets/<id>.svg`) rather than `public/`, for the same reason
...
- Designs are no longer static config either - manage them from the admin
  ...
  is a fixed, static image (it does not recolor with the Accent Colour
  picker) - only the initials text (when Zone's "follows Accent Colour"
  is on) and the motif icon drawn inside Motif Zone pick up the
  customer's chosen accent.
