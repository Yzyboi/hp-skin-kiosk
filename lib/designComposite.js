...
// balloon the composited PNG file size well past anything useful, which
// is enough on its own to blow past email attachment size limits. Capped
// at exactly the 300 DPI print target (not higher) since this PNG is
// currently only used as the email's "screen reference" attachment - the
// CMYK PDF that used to need real print-resolution headroom is disabled
// (see lib/orderDelivery.js). If the PDF path is re-enabled, raise this
// back toward PRINT_PX_PER_MM * 2 so the print deliverable isn't capped
// at screen-reference quality.
const MAX_PRINT_PX_PER_MM = PRINT_PX_PER_MM;
