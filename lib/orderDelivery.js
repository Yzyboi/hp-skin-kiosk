// Background order delivery: composites the print asset and emails the
// print provider - all off the customer's request/response cycle (see
// routes/kiosk.js POST /submit). The customer's confirmation no longer
// waits on any of this; it only waits on the order being durably
// persisted.
//
// The CMYK PDF (lib/printAsset.js) is not attached here - it's disabled
// for now, see the comment above sendOneAttempt().
//
...
// PDF attachment disabled for now - only the PNG goes out. Re-enable by
// building cmykPdfBuffer (see lib/printAsset.js's buildCmykPdf) in
// deliverOrder()/retrySweepOrder() below and passing it through here and
// to sendSpecSheetEmail again.
async function sendOneAttempt({ referenceId, toAddresses, specSheet, previewPngBuffer, attempt }) {
  try {
    await sendSpecSheetEmail({ toAddresses, specSheet, previewPngBuffer });
...
async function deliverOrder({ referenceId, store, specSheet, design, sku, initials }) {
  ...
  try {
    previewPngBuffer = await compositeDesignPng({ design, sku, initials, referenceId });
  } catch (err) {
...
async function retrySweepOrder(order) {
  ...
  let previewPngBuffer;
  try {
    previewPngBuffer = await compositeDesignPng({ design, sku, initials: order.initials, referenceId: order.referenceId });
  } catch (err) {
