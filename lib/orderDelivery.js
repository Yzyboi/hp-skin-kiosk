// Background order delivery: composites the print asset, converts it to
// a print-ready CMYK PDF, and emails that PDF to the print provider -
// all off the customer's request/response cycle (see routes/kiosk.js
// POST /submit). The customer's confirmation no longer waits on any of
// this; it only waits on the order being durably persisted.
//
// Only the PDF is attached (see lib/printAsset.js) - not the PNG. The
// composited PNG is still generated as the PDF's source image, it just
...
// Caps how many orders' compositing+PDF work (the memory-heavy sharp/
// pdfkit steps) run at the same time, across every store hitting this one
// shared process - see concurrencyLimiter.js.
const HEAVY_WORK_CONCURRENCY = 2;
const runHeavyWork = createLimiter(HEAVY_WORK_CONCURRENCY);
...
// Logged with name/statusCode (not just message) since Resend's error
// `name` (e.g. "invalid_from_address", "rate_limit_exceeded") tells you
// whether this is a config problem worth fixing immediately or a
// transient one the retry will likely clear on its own - see
// node_modules/resend's ErrorResponse shape.
...
async function sendOneAttempt({ referenceId, toAddresses, specSheet, cmykPdfBuffer, attempt }) {
  try {
    await sendSpecSheetEmail({ toAddresses, specSheet, cmykPdfBuffer });
...
async function deliverOrder({ referenceId, store, specSheet, design, sku, initials }) {
  ...
  try {
    previewPngBuffer = await runHeavyWork(() => compositeDesignPng({ design, sku, initials, referenceId }));
  } catch (err) {
  ...
  let cmykPdfBuffer;
  try {
    cmykPdfBuffer = await runHeavyWork(() =>
      buildCmykPdf({ pngBuffer: previewPngBuffer, widthMm: sku.widthMm, heightMm: sku.heightMm })
    );
  } catch (err) {
    console.error(`[order ${referenceId}] CMYK PDF generation failed:`, err.message);
    updateOrder(referenceId, {
      emailStatus: "failed",
      emailError: `Could not generate the print-ready PDF: ${err.message}`
    });
    return;
  }
...
// rebuilds everything (spec sheet, artwork composite) from the persisted
// order record itself rather than from any in-memory state.
async function retrySweepOrder(order) {
  ...
  let previewPngBuffer;
  try {
    previewPngBuffer = await runHeavyWork(() =>
      compositeDesignPng({ design, sku, initials: order.initials, referenceId: order.referenceId })
    );
  } catch (err) {
  ...
  try {
    ...
    updateOrder(order.referenceId, {
      emailStatus: "failed",
      emailError: `Could not regenerate the print-ready PDF: ${err.message}`
    });
    return;
  }
