// Runtime access to the design catalog (data/designs.json + the SVG/PNG
...
// and the uploaded artwork bytes survive restarts/redeploys on a host
// with a persistent disk (see README "Hosting").
//
// The uploaded SVG or PNG file is stored and served byte-for-byte as
// given - the admin flow never re-encodes, minifies, recolors, or
// rasterizes it, since the base artwork must render exactly as
// submitted.
...
// Extracts the file extension ("svg" or "png") a design's assetPath was
// last saved with, e.g. "/design-assets/shear.svg" -> "svg".
function extFromAssetPath(assetPath) {
  const match = /\.([a-z0-9]+)$/i.exec(String(assetPath || ""));
  return match ? match[1].toLowerCase() : "svg";
}

// Adds a new design: writes the raw artwork bytes to disk untouched and
// appends the JSON record. fields: { name, zone, fileBuffer, assetType }.
// fileBuffer: raw bytes of the uploaded .svg/.png file.
// assetType: "svg" or "png", as detected from the file's own content.
function addDesign({ name, zone, fileBuffer, assetType }) {
  const designs = readDesigns();
  const existingIds = new Set(designs.map((d) => d.id));
  const id = uniqueIdFromName(name, existingIds);
  const now = new Date().toISOString();

  fs.writeFileSync(assetFilePath(id, assetType), fileBuffer);

  const record = {
    id,
    name,
    assetPath: `/design-assets/${id}.${assetType}`,
    zone,
    updatedAt: now
  };
  designs.push(record);
...
// Updates an existing design's fields. If fileBuffer is provided, the
// artwork file is replaced (still byte-for-byte, no re-encoding) - if
// the new file is a different format than the old one (e.g. swapping an
// SVG for a PNG), the old artwork file is removed and assetPath is
// updated to the new extension. If no fileBuffer is given, the existing
// asset is left as-is. Returns the updated record, or null if no design
// with that ID exists.
function updateDesign(id, { name, zone, fileBuffer, assetType }) {
  const designs = readDesigns();
  const existing = designs.find((d) => d.id === id);
  if (!existing) return null;

  if (fileBuffer) {
    const oldExt = extFromAssetPath(existing.assetPath);
    if (oldExt !== assetType) {
      const oldPath = assetFilePath(id, oldExt);
      if (fs.existsSync(oldPath)) fs.unlinkSync(oldPath);
    }
    fs.writeFileSync(assetFilePath(id, assetType), fileBuffer);
    existing.assetPath = `/design-assets/${id}.${assetType}`;
  }
  existing.name = name;
  existing.zone = zone;
...
function deleteDesign(id) {
  ...
  const assetPath = assetFilePath(id, extFromAssetPath(existing.assetPath));
