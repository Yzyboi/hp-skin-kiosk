...
//
// The CMYK PDF (lib/printAsset.js) is not attached - disabled for now,
// see the comment in lib/orderDelivery.js's sendOneAttempt().
//
...
async function sendSpecSheetEmail({ toAddresses, specSheet, previewPngBuffer }) {
  ...
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#111;">
      ...
      <p style="color:#666;margin-top:16px;">Preview attached as PNG (screen reference).</p>
    </div>
  `;
  ...
  const attachments = [
    {
      ...
      content: previewPngBuffer.toString("base64"),
