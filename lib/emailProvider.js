...
function getTransporter() {
  if (!transporter) {
    transporter = nodemailer.createTransport({
      ...
      // but socketTimeout in particular has to allow for a slow upload of
      // the PNG + CMYK PDF attachments over a constrained connection - too
      // tight here produces a false failure (nodemailer times out waiting
      // for the final SMTP response even though the message was already
      // fully accepted server-side and gets delivered anyway).
      connectionTimeout: 15000,
      greetingTimeout: 15000,
      socketTimeout: 45000,
      // Without pooling, every single order pays a fresh TCP+TLS+auth
      // handshake to the SMTP server before it can even start uploading
      // the attachments - that's most of the "Submitting..." wait. Pooling
      // keeps a small set of authenticated connections open and reuses
      // them across orders, so only the very first order after a cold
      // start (or after maxMessages/idle timeout recycles a connection)
